...
import { getServiceLocationsThunk } from '../../redux/thunks/services/contractRoutes/contractRoutesThunk';
import { Loading } from '../../screens/utilities/Loading';

export const ScheduleCompanyServiceForm = () => {
  const { serviceLocations, isLoading } = useAppSelector(state => state.services);
  ...
  useEffect(() => {
    dispatch(getServiceLocationsThunk());
  }, [])

  useEffect(() => {
    const locations: Address[] = serviceLocations?.map(description => ({
      description,
      latitude: 0,
      longitude: 0
    }));
